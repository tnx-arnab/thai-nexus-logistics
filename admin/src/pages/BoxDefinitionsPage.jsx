...
const BoxDefinitionsPage = () => {
  ...
  const fetchBoxes = async () => {
    ...
    try {
      // @ts-ignore
      const response = await axios.get(window.tnxData.apiUrl + '/box-definitions', {
        headers: {
          // @ts-ignore
  ...
  const handleSave = async () => {
    ...
    try {
      // @ts-ignore
      await axios.post(window.tnxData.apiUrl + '/box-definitions', { boxes }, {
        headers: {
          // @ts-ignore
          'X-WP-Nonce': window.tnxData.nonce
        }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={addBox}
          className="tnx-btn-primary py-2.5 px-6 flex items-center gap-2"
        >
      ...
      {boxes.length === 0 ? (
        <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-20 flex flex-col items-center justify-center text-center">
          ...
          <button
            onClick={addBox}
            className="tnx-btn-primary py-2.5 px-8"
          >
            Add First Box
          </button>
        </div>
        ...
        <div className="space-y-6">
          {boxes.map((box, index) => (
            <div key={index} className="tnx-card group overflow-visible">
