...
const DebugLogPage = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      // @ts-ignore
      const response = await axios.get(window.tnxlData.apiUrl + '/debug-log', {
        headers: {
          // @ts-ignore
  ...
  const clearLogs = async () => {
    try {
      // @ts-ignore
      await axios.delete(window.tnxlData.apiUrl + '/debug-log', {
        headers: {
          // @ts-ignore
          'X-WP-Nonce': window.tnxlData.nonce
        }
  ...
  const clearCache = async () => {
    try {
      // @ts-ignore
      await axios.delete(window.tnxlData.apiUrl + '/cache', {
        headers: {
          // @ts-ignore
          'X-WP-Nonce': window.tnxlData.nonce
        }
  ...
  return (
    <div className="space-y-6">
      ...
      {logs.length === 0 ? (
        <div className="tnxl-card p-20 flex flex-col items-center justify-center text-center">
        ...
        <div className="space-y-4">
          {logs.map((log) => (
            <div key={log.id} className="tnxl-card overflow-hidden transition-all duration-200">
              {/* Log Header */}
              <div 
