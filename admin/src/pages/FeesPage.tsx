...
import type { CommissionRule, PricingMode } from '../lib/types';
...
export default function FeesPage() {
    ...
    const load = useCallback(async () => {
        ...
        const nextRules = (data.commission_rules || data.commissionRules || []).map(
            (r: CommissionRule) => ({ ...r })
        );
        setRules(nextRules);
        setMode(inferPricingMode(nextRules, data.pricing_mode || data.pricingMode));
        ...
        setCurrency(data.currency_symbol || data.currencySymbol || '฿');
        setWizard(null);
        setComposing(false);
        setFormEpoch((n) => n + 1);
        setLoaded(true);
                    ...
                    {mode === 'basic' ? (
                        <div className="space-y-4 max-w-3xl">
                            ...
                            <PricingFormulaEditor
                                ...
                                onChange={patchBasic}
                                currency={currency}
                                draftKey={`${basicRule.id}-${formEpoch}`}
                                example={basicExample}
                                showCart={cartOpen || basicFormula.cartPercent > 0}
                                onToggleCart={() => setCartOpen(true)}
                            />
                            <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3 text-sm text-gray-600" dir="ltr">
