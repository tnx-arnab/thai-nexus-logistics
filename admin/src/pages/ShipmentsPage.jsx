...
const ShipmentsPage = () => {
  ...
  const fetchShipments = async () => {
    ...
    try {
      // @ts-ignore
      const response = await axios.get(`${window.tnxlData.apiUrl}/shipments?page=${page}&limit=10`, {
        headers: {
          // @ts-ignore
  ...
  const fetchShipmentDetails = async (requestNumber) => {
    ...
    try {
      // @ts-ignore
      const response = await axios.get(`${window.tnxlData.apiUrl}/shipments/${requestNumber}`, {
        headers: {
          // @ts-ignore
          'X-WP-Nonce': window.tnxlData.nonce
        }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="tnxl-card transition-all hover:shadow-sm">
