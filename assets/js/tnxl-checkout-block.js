(function() {
    ...
    const initTnxlMonitoring = () => {
        ...
        if (window.wc && window.wc.blocksCheckout) {
            ...
            const tnxlShippingFilter = ( shippingRates ) => {
                return shippingRates;
            };

            registerCheckoutFilters( 'tnxl-shipping', {
                shippingRates: tnxlShippingFilter,
            } );
        }
    };
    ...
    if (window.wp && window.wp.domReady) {
        window.wp.domReady(initTnxlMonitoring);
    } else {
        document.addEventListener('DOMContentLoaded', initTnxlMonitoring);
    }
})();